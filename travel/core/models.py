...
class Place(models.Model):
    ...
    phones = models.CharField(max_length=10000,
                              verbose_name='Телефоны заведения',
                              blank=True, null=True)
    website_url = models.CharField(max_length=10000,
                                   verbose_name='Ссылка на сайт',
                                   null=True, blank=True)
    working_hours = models.CharField(max_length=10000,
...
class PlaceUserRatingManager(models.Manager):
    ...
    def add_rating(self, place, user, rating, review):
        ...
        rating = PlaceUserRating(user=user,
                                 place=place,
                                 rating=rating,
                                 review=review)
    ...
    def update_user_ratings(self, place, user):
        approved_user_ratings = PlaceUserRating.objects.filter(
            place=place,
            user=user,
            status=constants.FEEDBACK_APPROVED)
        copy, _ = PlaceUserRatingRequest.objects.get_or_create(
            place=place,
            user=user)
